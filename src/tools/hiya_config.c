/*
 * hiya_config.c — Hiya Configuration Tool
 *
 * Copyright (C) 2024 Hiya Project
 * SPDX-License-Identifier: GPL-3.0-or-later
 *
 * CLI tool for viewing and modifying Hiya configuration.
 *
 * Usage:
 *   hiya-config [COMMAND] [OPTIONS]
 *
 * Commands:
...
static const config_entry_t config_defaults[] = {
    ...
    {"general.state_dir", "/var/lib/hiya",
    ...
    {"fido2.storage_path", "/var/lib/hiya/fido2",
...
static int config_save(const config_t *cfg)
{
    ...
    fprintf(f, "# Hiya Configuration\n");
    fprintf(f, "# Generated by hiya-config\n");
    fprintf(f, "# See hiya-config show --defaults for all options.\n\n");
...
{
    config_t cfg;
    config_load(&cfg, HIYA_CONFIG_FILE);

    printf("╔══════════════════════════════════════════════════╗\n");
    printf("║           Hiya Configuration                 ║\n");
    printf("╚══════════════════════════════════════════════════╝\n\n");
...
static int cmd_set(const char *key, const char *value)
{
    ...
    printf("Apply changes with: systemctl kill -s HUP hiya-authd.service\n");
    return 0;
}
...
static int cmd_reset(void)
{
    ...
    config_t cfg;
    memset(&cfg, 0, sizeof(cfg));
    strncpy(cfg.path, HIYA_CONFIG_FILE, sizeof(cfg.path) - 1);
...
static int cmd_status(void)
{
    ...
    struct stat st;
    if (stat("/run/hiya/fido2.sock", &st) == 0 && S_ISSOCK(st.st_mode))
        printf("/run/hiya/fido2.sock (active)\n");
    else
        printf("not found\n");

    /* Check config file */
    printf("  %-30s ", "Config file:");
    if (access(HIYA_CONFIG_FILE, R_OK) == 0)
        printf("%s\n", HIYA_CONFIG_FILE);
    else
...
static int cmd_fido2_info(void)
{
    ...
    printf("  Socket:      ");
    if (stat("/run/hiya/fido2.sock", &st) == 0 && S_ISSOCK(st.st_mode))
        printf("/run/hiya/fido2.sock (listening)\n");
    else
...
static void usage(const char *prog)
{
    fprintf(stderr,
            ...
            "  %s status\n"
            "  %s fido2-info\n",
            prog, HIYA_CONFIG_FILE, prog, prog, prog, prog, prog);
}
